...
class PaquitoCodecFactoryTest < PaquitoTest
  ...
  test "all types are stable together" do
    assert_equal(
      ...
      "\x00\x00\x01\x00\x00\x00\xD6\x00date\xD6\x03\xD0\a\x01\x01\xD6\x00hash\x81\xD4\x00a\x91\xD4\x00a".b,
      @codec.dump(
        symbol: :symbol,
        string: "string",
        array: [:a, "b"],
  ...
  test "payloads are forward compatible" do
    ...
    assert_equal(expected, @codec.load(
      ...
      "\xD6\x00time\xC7\f\x01\xCA\x19m8\x00\x00\x00\x00\x01\x00\x00\x00\xD7\x00datetime\xC7\f\x01\xA26m8\x00\x00\x00" \
      "\x00\x01\x00\x00\x00\xD6\x00date\xD6\x03\xD0\a\x01\x01\xC7\n\x00bigdecimal\xC7\n\x0427:0.123e3\xD6\x00hash" \
