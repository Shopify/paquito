...
module Paquito
  module SharedCodecFactoryTests
    ...
    V1_PAYLOAD = "\x87\xC7\x06\x00symbol\xC7\x06\x00symbol\xC7\x06\x00string\xA6string\xC7\x05\x00array\x92\xD4" \
      "\x00a\xA1b\xD6\x00time\xC7%\v\x04\bIu:\tTime\r\"\x00\x19\x80\x00\x00 \b\a:\voffseti\x00:\tzone0\xD7\x00" \
      "datetime\xC7'\f\x04\bU:\rDateTime[\vi\x00i\x03Yh%i\x02r9i\x00i\x00f\f2299161\xD6\x00date\xD6\x03\xD0\a" \
      "\x01\x01\xD6\x00hash\x81\xD4\x00a\x91\xD4\x00a".b.freeze
  ...
  class CodecFactoryV1Test < PaquitoTest
    ...
    test "issue#26 version 1 Time serializer handle arbitrary size denominators" do
      time = Time.at(Rational(1, 2**33))
      time_state = time.inspect

      time_copy = @codec.load(@codec.dump(time))
      assert_equal time_state, time_copy.inspect
    end
